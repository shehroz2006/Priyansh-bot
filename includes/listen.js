...
(async function () {
  ...
  logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "This bot was made by Robin" : global.config.BOTNAME}`, "[ BOT INFO ]");
  ...
  const checkAndExecuteEvent = async () => {
    ...
    //GET CURRENT TIME
    var timeVN = moment().tz('Asia/Karachi').format('DD/MM/YYYU_HH:mm:ss');
  ...
  return (event) => {
    ...
    let data = JSON.parse(fs.readFileSync(__dirname + "/../Priyansh/commands/Robin/approvedThreads.json"));
...
😳𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝🤓:- ☞ www.facebook.com/61570766535522\n

📑 Agar Approval Nahi 🙅🏻‍♂️ Mil Raha Hai To Mere Øwner🖤 Ko Direct Add Kar Sakte ho Fb Id link se 😊💖\n

👋For Any Kind Of Help Contact On Wha😇`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
